...
export class OwnerAuthGuard implements CanActivate {
  // PostsService를 주입받아 게시글 정보를 조회
  constructor(private readonly postsService: PostsService) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context.switchToHttp().getRequest();
    
    // JWT Strategy를 통해 req.user에 담긴 사용자 정보를 가져옴.
    const user = request.user; 
    
    // 요청 경로에서 게시글 ID를 가져옴. (PATCH /posts/:id 에서 :id)
    const postId = request.params.id; 

    if (!user || !postId) {
        // 이 Guard는 JwtAuthGuard 뒤에 실행되므로, user가 없으면 Forbidden을 반환.
        throw new ForbiddenException('인증된 사용자 정보가 누락되었습니다.');
    }

    // PostsService를 통해 게시글 정보를 가져옴.
