...
export class AuthController {
  ...
  /**
   * 1. IDP 로그인 확인 및 DB 저장
   * Postman에서 Bearer 토큰을 헤더에 담아 호출하면 
   * Strategy가 IDP에 물어보고 우리 DB에 유저를 저장한 뒤 정보를 반환한다. 
   */
  @ApiOperation({ summary: 'IDP 로그인 상태 확인 및 유저 저장' })
  @ApiResponse({ status: 200, description: '로그인 성공 및 DB 저장 완료' })
  @Get('profile')
  @UseGuards(AuthGuard('infoteam')) // 우리가 만든 'infoteam' strategy 사용
  async getProfile(@Request() req: any) {
    // Strategy의 validate()가 반환한 유저 정보가 req.user에 들어있음.
    return {
  ...
  /**
   * 2. 로그아웃
   * 토큰 인증 방식은 서버에 세션이 없으므로, 클라이언트가 토큰을 삭제하면 된다.
   * 로그아웃 성공 메시지만 반환란다.
   */
